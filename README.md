...
OpenAI #1 (gpt-3.5-turbo)  ───  returns lead score, reason, and website guess as JSON
        │                  └── [error handler] → Gmail: notifies me with the business name + error details if this fails
        ▼
...
(Create      │   drafts a suggested outreach email
 Record)     │   └── [error handler] → Gmail: notifies me with the business name + error details if this fails
             ▼
...
| 3 | OpenAI #1 – Create Chat Completion | Takes one business's Name, Address, Rating, and Total Ratings and returns **strict JSON**: `{"score": <1-10>, "reason": "<one sentence>", "has_website": <true/false>}`. Has an attached error handler (see below). |
| 3a | *(error handler)* Gmail – Send an Email | Fires only if step 3 fails. Emails me the business name and the specific error type/message, so a failure is visible immediately instead of silently producing empty data. |
...
| 7b | OpenAI #2 – Create Chat Completion | Drafts a short (3-4 sentence) personalized outreach email with subject line, for leads scoring below 7. Has an attached error handler (see below). |
| 7b-i | *(error handler)* Gmail – Send an Email | Fires only if step 7b fails. Same pattern as the first error handler — business name + error details, sent immediately. |
...
- **No error handling anywhere in the scenario.** Confirmed directly: when my OpenAI account ran out of API credit mid-test, the OpenAI module failed with a `429 RateLimitError`, but nothing downstream was notified or stopped cleanly — the Iterator's bundles just came through empty, and every module after it silently processed blank data with no error surfaced until I manually checked each module's execution log. **Fix (partial):** added dedicated error handlers on both OpenAI modules — the two steps most likely to fail, since they're the only ones dependent on OpenAI API credit/availability. Each handler sends an immediate email with the specific business being processed and the exact error type/message, so a failure is visible in real time instead of being discovered later by digging through logs. Tested live against the actual credit-exhaustion error and confirmed the notification works correctly.

### Still open

- **Error handling isn't complete everywhere.** Only the two OpenAI modules have error handlers so far. Google Maps, Google Sheets, Airtable, Google Docs, and the outbound Gmail step still fail silently. Google Maps in particular deserves attention — a search returning zero results isn't a "failure" Make.com would flag on its own, so it needs a different pattern (a check/filter after the module, not a standard error handler) to catch and report.
...
1. Extend error handling to the remaining modules (Maps, Sheets, Airtable, Docs, Gmail), and add a zero-results check after Google Maps specifically, since that's a silent "success" rather than a flagged error.
...
- OpenAI's API has no free tier — separate from ChatGPT's web interface — and requires prepaid credit to make any call at all, confirmed via a `429 RateLimitError` when credit ran out mid-test. This same real failure became the test case used to build and verify the error handlers described above.
...
- Make.com error handler branches expose the actual error object (`Type`, `Message`, `Detail`) from the failed module, which can be mapped directly into a notification — this made it possible to build a specific, actionable alert rather than a generic "something failed" message.
- Gmail's "Raw HTML" body type ignores plain-text line breaks; `<br>` tags are needed to format a multi-line notification email correctly.
